...
export default async function SimilarPage(
  ...
  const result = await TMDB.getSimilar(mediaId, parsedType, parseInt(page + "") || 1);
  ...
  return (
    <PageWrapper>
      <ResultsTopBar totalPages={Math.min(result.ok.total_pages, 500)} title={`Similar to "${name}"`} buttons={[]} />
      <BlocksWrapper>
